...
class OneMusucian {
 ...
 public:
  ...
  // Check for blocking only for current candidate, ignore what it will block
  // previous musicians
  static bool FindBestLocationEarlyStop3_PB(
      const Problem& p, unsigned instrument, const D2Point& start_point,
      const std::vector<D2Point>& selected, OneMusucian& current_best,
      bool print_log = false, unsigned iteration = 0) {
    if (print_log) {
    ...
    auto score = Evaluator::DScoreMusician(p, instrument, pos, selected);
    ...
    for (;;) {
      ...
      D2Vector v;
      for (auto& a : p.attendees) {
        if (Evaluator::Blocked(a, pos, p, selected)) continue;
        D2Vector v0 = a.position - pos;
        auto v0ls = v0.LengthSquared();
        v += v0 * (score_mult * a.tastes[instrument] / (v0ls * v0ls));
      }
      ...
      auto f = [&](double x) {
        return -Evaluator::DScoreMusician(p, instrument, pos + v * x, selected);
      };
      auto x0 = opt::Minimum(f, 0, 1.0, 1e-6);
      pos += v * x0;
      auto new_score = Evaluator::DScoreMusician(p, instrument, pos, selected);
      // if (print_log)
      //   std::cout << "\t" << score << " -> " << new_score << std::endl;
      if (new_score <= score + 1e-6) {
        if (print_log) {
